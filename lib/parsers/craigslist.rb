module Craigslist
  ...
  def self.parse(options = {})
    ...
    # scrape title
    title = get_title(doc) if fields.blank? || fields.include?("price") || fields.include?("title")
    
    # scrape description
    description = get_description(doc) if fields.blank? || fields.include?("price") || fields.include?("description")
    
    # scrape price
    price = get_price(title, description) if fields.blank? || fields.include?("price")
    
    # scrape posted time
    posted_at = get_posted_time(doc) if fields.blank? || fields.include?("posted_at")
    
    # main image
    image = get_image(doc) if fields.blank? || fields.include?("image")
    
    # scrape email
    email = get_email(doc) if fields.blank? || fields.include?("email")
    
    # phone number
    phone = get_phone(description) if fields.blank? || fields.include?("phone")
  ...
  def self.get_price(title, description = nil)
    description ||= ""
  ...
  def self.cl_city_site_info(region)
    usa = [{
      "state" => "AL",
      "city_code" => "auburn",
      "city" => "auburn, AL",
